...
app.get('/api/users/record/:userId', verifyToken, async (req, res) => {
	try {
		...
		const record = await knex('users')
			.where({ id: userId })
			.then((ArrOfData) => {
				// recordがオブジェクトの場合、空の配列を使用
				const ArrOfSpot = Array.isArray(ArrOfData[0].record) ? ArrOfData[0].record : [];
...
app.post('/api/newspot', verifyToken, async (req, res) => {
	try {
		...
		const spotId = await knex('spot').insert({ name, area, mission, photo, longitude, latitude }).returning('id');
...
app.get('/api/spots/:userId', async (req, res) => {
	try {
		...
		// recordが配列でなければ空の配列を使用
		const userRecords = Array.isArray(records[0].record) ? records[0].record : [];
		...
		if (!area) {
			...
			for (const spot of spots) {
				...
				const rec = userRecords.filter((record) => record.spot_id === spot.id);
				// record内容をspotに追記
				if (rec.length === 0) {
					spot.finish = false;
				} else {
